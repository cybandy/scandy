...
def host_stat(ip):
    ...
    if "windows" in pl.lower():
        pinging = str(subprocess.run(['ping','-n','1',ip], capture_output=True).stdout).lower()
        if "received = 0" not in pinging:
            return True if "unreachable" not in pinging else False
        return False
        # return "received = 0" not in pinging and "unreachable" not in pinging
    else:
        pinging = str(subprocess.run(['ping', '-c', '1', ip], capture_output=True).stdout).lower()
        if "0 received" not in pinging:
            return True if "unreachable" not in pinging else False
        return False
        # return "0 received" not in pinging
